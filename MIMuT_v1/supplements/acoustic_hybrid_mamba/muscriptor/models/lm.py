...
class LMModel(nn.Module):
    ...
    def forward(
        ...
        if first_step:
            if self.type_embedding is not None:
                ...
                ordered = [k for k in preferred if k in condition_tensors]
                ordered.extend(k for k in condition_tensors if k not in ordered)
                chunk = self.chunk_start.expand(B, -1, -1)
                chunk = chunk + self.type_embedding(
                    torch.zeros(B, 1, dtype=torch.long, device=input_.device)
                )
                pieces = [chunk]
                for key in ordered:
                    cond, mask = condition_tensors[key]
                    # Conditioners run in fp32 even when the backbone runs in
                    # half precision (mel numerics degrade in fp16).
                    cond = cond.to(input_.dtype)
                    ...
                    cond = cond * mask.unsqueeze(-1).to(cond.dtype)
                    pieces.append(cond)
                pieces.append(input_)
                input_ = torch.cat(pieces, dim=1)
            else:
    ...
    def generate(
            ...
            num_samples = (
                ...
                if conditions
                    ...
                    if prompt is not None
                        ...
                        next(iter(condition_tensors.values()))[0].shape[0]
                        // (1 if cfg_coef == 1.0 else 2)
        ...
        if condition_tensors is not None:
            if conditions:
                raise ValueError(
                    "pass either raw conditions or pre-encoded condition_tensors, not both"
                )
        ...
        if model_state is None:
            # Allocate state only for the autoregressive decoder subtree.
            # Acoustic-Mamba owns a separate whole-song state dictionary in
            # TranscriptionModel.  Walking ``self`` here would rename its
            # StatefulModule slots and silently break acoustic carry after
