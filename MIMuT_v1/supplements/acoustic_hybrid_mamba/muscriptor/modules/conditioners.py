...
class MelSpectrogramConditioner(nn.Module):
    ...
    def __init__(
        ...
        self.acoustic_encoder: nn.Module | None = None
        if model_config is not None and model_config.acoustic_encoder == "mamba2":
            from muscriptor.modules.acoustic import AcousticMambaEncoder

            self.acoustic_encoder = AcousticMambaEncoder(
                model_config,
                device=device,
            )
    ...
    @property
    def has_streaming_encoder(self) -> bool:
        return self.acoustic_encoder is not None

    def set_gradient_checkpointing(self, enabled: bool = True) -> None:
        if self.acoustic_encoder is not None:
            self.acoustic_encoder.set_gradient_checkpointing(enabled)
    ...
    def forward(
        ...
        *,
        ...
        if self.acoustic_encoder is not None:
            flat_batch, frames, dim = embeds.shape
            if batch_shape is not None:
                batch, chunks = batch_shape
                if flat_batch != batch * chunks:
                    raise ValueError(
                        "acoustic batch shape does not match flattened conditions: "
                        f"{flat_batch} != {batch} * {chunks}"
                    )
                embeds = embeds.reshape(batch, chunks * frames, dim)
                sequence_mask = mask.reshape(batch, chunks * frames)
            else:
                sequence_mask = mask

            encoder_parameter = next(self.acoustic_encoder.parameters())
            embeds = embeds.to(encoder_parameter)
            embeds = self.acoustic_encoder(embeds, model_state=model_state)
            embeds = embeds * sequence_mask.unsqueeze(-1).to(embeds.dtype)
            if model_state is not None:
                increment_steps(
                    self.acoustic_encoder,
                    model_state,
                    increment=embeds.shape[1],
                )

            if batch_shape is not None:
                embeds = embeds.reshape(flat_batch, frames, dim)
        return embeds, mask
...
class ClassConditioner(nn.Module):
    ...
    def __init__(
        ...
        self.pad_idx = 0
        self.correct_padding = correct_padding
    ...
    def forward(self, inputs: torch.Tensor) -> ConditionType:
        if self.correct_padding:
            embeds = self.embed(inputs)
            mask = inputs.ne(self.pad_idx).to(torch.long)
            embeds = embeds * mask.unsqueeze(-1)
        else:
            # Historical behavior retained for released checkpoints.
...
class ConditioningProvider(nn.Module):
    ...
    def forward(
        ...
        *,
        acoustic_state: ModelState | None = None,
        acoustic_batch_shape: tuple[int, int] | None = None,
    ) -> dict[str, ConditionType]:
        output = {}
        for attribute, inputs in tokenized.items():
            conditioner = self.conditioners[attribute]
            if isinstance(conditioner, MelSpectrogramConditioner):
                condition, mask = conditioner(
                    inputs,
                    model_state=acoustic_state,
                    batch_shape=acoustic_batch_shape,
                )
            else:
                condition, mask = conditioner(inputs)
