...
class ClassConditioner(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.device = device
        # New checkpoints reserve 0 for sequence padding and 1 for a learned
        # NULL condition.  Legacy checkpoints retain their historical table
        # and index shift exactly.
        if learned_null and not correct_padding:
            raise ValueError("learned_null requires correct_padding")
        table_size = num_classes + (2 if learned_null else 1)
        self.embed = nn.Embedding(table_size, output_dim).to(device)
        self.pad_idx = 0
        self.null_idx = 1
        self.correct_padding = correct_padding
        self.learned_null = learned_null

    def tokenize(self, x: list[str | None]) -> torch.Tensor:
        if self.learned_null:
            # 0=padding, 1=learned NULL, class k=(k+2).
            int_x = [
                [self.null_idx]
                if value is None
                else [int(item) + 2 for item in value.split()]
                for value in x
            ]
            max_len = max(len(values) for values in int_x)
            int_x = [
                values + [self.pad_idx] * (max_len - len(values))
                for values in int_x
            ]
            return torch.LongTensor(int_x).to(self.device)
        ...
        return 1 + torch.LongTensor(int_x).to(self.device)
