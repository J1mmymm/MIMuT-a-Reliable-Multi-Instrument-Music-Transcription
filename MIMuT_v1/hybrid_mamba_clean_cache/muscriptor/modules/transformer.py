...
class StreamingMultiheadAttention(StatefulModule):
    ...
    def reorder_state(self, state: State, indices: torch.Tensor) -> None:
        cache = state["cache"]
        state["cache"] = cache.index_select(1, indices.to(cache.device))
    ...
    def forward(
        ...
        if T_q == 1:
            ...
            # A retained-state Transformer sees this shape at the start of
            # every new audio chunk: a multi-token condition prefix follows
            # the previous chunk's cache. Build a bottom-right causal mask.
            # This path is intentionally full-history and serves as the
            # quadratic long-context control for Hybrid-Mamba.
            past = T_k - T_q
            query_positions = torch.arange(past, T_k, device=q_t.device).view(-1, 1)
            key_positions = torch.arange(T_k, device=q_t.device).view(1, -1)
            allowed = key_positions <= query_positions
            x = F.scaled_dot_product_attention(
                q_t,
                k_t,
                v_t,
                attn_mask=allowed.view(1, 1, T_q, T_k),
                dropout_p=0.0,
            )
