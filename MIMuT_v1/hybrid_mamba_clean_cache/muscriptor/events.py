...
class OpenNoteTracker:
    ...
    def fork(self) -> "OpenNoteTracker":
        """Create a transaction-local tracker with independent containers."""

        return deepcopy(self)

    @property
    def commit_ready(self) -> bool:
        """Whether a normally terminated chunk may update committed state."""

        return bool(
            self._chunk_started
            and not self._in_prologue
            and not self._skip_rest
