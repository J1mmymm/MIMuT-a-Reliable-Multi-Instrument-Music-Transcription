"""CUDA/Mamba/FlashAttention acceptance check for the training machine."""
...
from muscriptor.models.config import (
    ModelConfig,
    estimate_backbone_parameters,
)
from muscriptor.modules.hybrid import StreamingMixedBackbone
from muscriptor.modules.hybrid import StreamingLocalAttention
...
def run_check() -> dict[str, object]:
    ...
    model = StreamingMixedBackbone(config, device=device, dtype=torch.bfloat16)
    flash_attention_active = any(
        isinstance(module, StreamingLocalAttention)
        and module._flash_attention is not None
        for module in model.modules()
    )
    if not flash_attention_active:
        raise RuntimeError("FlashAttention sliding-window kernel is not importable")
    actual_parameters = sum(parameter.numel() for parameter in model.parameters())
    estimated_parameters = estimate_backbone_parameters(config)
    ...
    if not torch.isfinite(training_input.grad).all():
        raise RuntimeError("non-finite gradient from Hybrid-Mamba")

    # Exercise the retained-state path with multi-token chunk prefixes.  With
    # a window wider than the sequence it must match the parallel execution.
    model.eval()
    ...
    if any(length > config.local_window for length in local_cache_lengths):
        raise RuntimeError("local-attention cache exceeded its configured bound")
    ...
    return {
        ...
        "flash_attention_active": flash_attention_active,
        "backbone_parameters": actual_parameters,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compile and validate Hybrid-Mamba CUDA kernels"
    )
    parser.add_argument(
        "--output",
