...
PrefillMode = Literal["auto", "chunk", "step"]
...
class ModelConfig:
    ...
    learned_null_conditioning: bool = False
    clean_acoustic_cache: bool = False
    boundary_state_dim: int = 0
    num_symbolic_instrument_groups: int = 37
    num_symbolic_pitches: int = 128
    num_reentry_classes: int = 5
    ...
    def __post_init__(self) -> None:
        ...
        if self.prefill_mode not in {"auto", "chunk", "step"}:
            raise ValueError("prefill_mode must be auto, chunk, or step")
        if self.learned_null_conditioning and not self.correct_class_conditioning:
            raise ValueError(
                "learned_null_conditioning requires correct_class_conditioning"
            )
        if self.clean_acoustic_cache:
            if not self.use_type_embeddings:
                raise ValueError("clean_acoustic_cache requires type embeddings")
            if not self.learned_null_conditioning:
                raise ValueError("clean_acoustic_cache requires learned NULL conditions")
            if self.boundary_state_dim <= 0:
                raise ValueError("clean_acoustic_cache requires boundary_state_dim > 0")
        if self.boundary_state_dim < 0:
            raise ValueError("boundary_state_dim must be non-negative")
        if min(
            self.num_symbolic_instrument_groups,
            self.num_symbolic_pitches,
            self.num_reentry_classes,
        ) <= 0:
            raise ValueError("symbolic-state dimensions must be positive")
...
MODEL_PRESETS = {**LEGACY_PRESETS, **HYBRID_PRESETS}
...
def estimate_total_parameters(config: ModelConfig) -> int:
    ...
    condition_extra = 2 if config.learned_null_conditioning else 1
    total += (config.num_instrument_condition_classes + condition_extra) * dim
    total += (config.num_dataset_condition_classes + condition_extra) * dim
    if config.use_type_embeddings:
        total += 4 * dim + dim  # type table and chunk marker
    if config.clean_acoustic_cache:
        groups = config.num_symbolic_instrument_groups
        pitches = config.num_symbolic_pitches
        reentry = config.num_reentry_classes
        total += dim  # committed-symbolic-state marker
        total += groups * dim + pitches * dim + reentry * dim
        bottleneck = config.boundary_state_dim
        total += dim * bottleneck + bottleneck
        total += bottleneck * (groups * pitches) + groups * pitches
        total += bottleneck * (groups * reentry) + groups * reentry
    return total
