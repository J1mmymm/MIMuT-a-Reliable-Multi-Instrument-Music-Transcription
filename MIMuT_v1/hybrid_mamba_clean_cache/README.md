# MuScriptor Hybrid-Mamba

> [!CAUTION]
> **Historical MIMuT v1 source snapshot.** This directory records an early
> Hybrid-Mamba/Clean-Cache research route that did not achieve the model quality
> expected by the project team. It is preserved for provenance, comparison, and
> ablation work only. It is not the latest MIMuT implementation or a recommended
> production architecture.

This fork adds a long-context Hybrid-Mamba transcription backbone while
preserving the released MuScriptor Transformer checkpoints and CLI.

## MIMuT AAAI research track

The formal paper track is **MIMuT: Reliable Multi-Instrument Music
Transcription**.  Its implementation is audio-only by default: acoustic input
is retained while instrument and dataset metadata are null during formal
training, and evaluation refuses incomplete or oracle-labelled prediction
runs unless an explicit diagnostic override is supplied.

- Main 312M context-mix/RoPE v2 runbook: [`docs/MAIN_312M_V2.md`](docs/MAIN_312M_V2.md)
- Machine-readable experiment matrix: [`configs/formal_experiment_matrix.yaml`](configs/formal_experiment_matrix.yaml)
- Static protocol check: `python tools/validate_formal_protocol.py --manifest MANIFEST`

No formal paper results are stored in this repository yet. Engineering smoke
checks and training loss logs must not be reported as transcription results.

- `hybrid_mamba`: official Mamba2 → Mamba2 → bounded local attention.
- 5/10/20/40-second contiguous curriculum; recurrent state carries through an
  entire song at inference.
- `hybrid-small`, `hybrid-medium`, and `hybrid-large` experiment presets.
- Canonical manifests, production MT3_FULL_PLUS supervision encoding,
  deterministic DDP/FSDP training, long-horizon metrics, and efficiency tools.

Start with [the architecture](docs/ARCHITECTURE.md),
[data preparation](docs/DATA.md), and the
[2 + 1 × 96 GB remote checklist](docs/REMOTE_TRAINING.md). The original MuScriptor
usage below remains valid; Hybrid checkpoints additionally support:

```bash
muscriptor transcribe song.wav --model checkpoint.safetensors \
  --long-context carry
```

# Original MuScriptor usage
...
The unrelated demonstration frontend and media assets were intentionally
excluded from this historical training-source archive. The backend and command
line transcription code are retained.
